...
Wall::Wall() {
	...
	mesh = createPlane(1000, 200, 100);
	calculateNormalPerFace();
	calculateNormalPerVertex();
	bmpTexture(this->texture, "bmp/wood.bmp");
	meshToDisplayList();
}
...
GLuint Wall::getDisplayList() {
	return this->display;
}

GLuint Wall::getTexture() {
	return this->texture;
}

void Wall::bmpTexture(UINT texture, const char *file) {
...
Mesh* Wall::createPlane(int arena_width, int arena_depth, int arena_cell) {
	Mesh *me = new Mesh;
...
//normal per face
void Wall::calculateNormalPerFace() {
...
void Wall::calculateNormalPerVertex() {
